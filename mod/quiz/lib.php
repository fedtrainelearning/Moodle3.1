    ...
    if ($attempts = $DB->get_records('quiz_attempts',
        ...
        foreach ($attempts as $attempt) {
            ...
            if ($attempt->state != quiz_attempt::FINISHED) {
                // echo quiz_attempt_state_name($attempt->state); // Matt edit
                echo quiz_attempt_state_name($attempt->state).' - '.userdate($attempt->timemodified).'<br />'; // Matt edit
            } else {
                echo quiz_format_grade($quiz, $attempt->sumgrades) . '/' .
                        // quiz_format_grade($quiz, $quiz->sumgrades); // Matt edit
                        quiz_format_grade($quiz, $quiz->sumgrades).' - Time start: '.userdate($attempt->timestart).' - Time finish: '.userdate($attempt->timefinish).'<br />'; // Matt edit
            }
            // echo ' - '.userdate($attempt->timemodified).'<br />'; // Matt edit
        }
    } else {
